...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      const fin = await supabase.from("agendamentos").select("*");
      const ags = fin.data || [];
      novo.recebido = ags.reduce((s,a)=>s+Number(a.valor_sinal||0)+Number(a.valor_recebido||0)+Number(a.recebido||0),0);
      novo.saldo = ags.reduce((s,a)=>s+Number(a.saldo||0),0);
      setDados(novo);
    }
    carregar();
  }, []);
  ...
  return (
    <div className="grid-dashboard">
      ...
      <div className="dashboard-card"><h2>🛠️ Serviços</h2><h1>{dados.servicos}</h1></div>
